# Workflows

Orchestration definitions (see ProjectFounder-idea.md, Section 20 — Workflow Engine, and Section 149 — Workflow Contract).

Planned initial workflows (Section 143): `new-project`, `brainstorm`, `deep-research`, `existing-project`, `github-project`, `saas`, `webapp`, `pwa`, `mobile`, `desktop`, `cli`, `api`, `ai`, `agent`, `mcp`, `browser-extension`, `library`, `self-hosted`, `infrastructure`.

- [`new-project.md`](./new-project.md) — implemented for Phase 0 (capture + classify, stops at lifecycle CLASSIFIED).
- [`brainstorm.md`](./brainstorm.md) — implemented for Phase 1 (Discovery + Brainstorm, CLASSIFIED -> DISCOVERY).

Status: Phase 0 and part of Phase 1 implemented (agent-executed, no code — see `docs/DECISIONS.md` D005). All other planned workflows not yet added.
