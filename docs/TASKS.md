...
- [ ] `todo` — Wire and exercise a workflow for Gap Analysis → Feature Discovery → Requirements against `examples/bookmark-manager/`. Not blocked on Research (Phase 2) for every category — Gap Analysis/Feature Discovery can run today using internal signals only (competitive-positioning sub-cases marked "provisional" per their contracts) — deferred only because this round's scope was contracts, not a new workflow.
