...
- 14 of 20 engines in § 7 (Complete Engine Inventory) have no agent contract at all yet. Six now have a working, exercised slice: Project, Discovery, Brainstorm (§ 7.1–§ 7.3, `workflows/new-project.md`/`workflows/brainstorm.md`), and Gap Analysis, Feature Discovery, Requirements (§ 7.7–§ 7.9, `workflows/requirements.md`).
...
- `checks/`, `resources/` contain only READMEs describing what will eventually live there ("Status: not yet implemented"). `commands/` and `workflows/` now each have three real files (`new-project.md`, `brainstorm.md`, `requirements.md`); everything else planned for them is still just a README list.
...
- Every Feature Discovery tier call so far (`examples/bookmark-manager/`) was decidable from internal signals alone — `agents/product-agent.md`'s "mark provisional" escalation for a tier that genuinely needs competitive research has never actually fired.
- `docs/OPEN-QUESTIONS.md` Q10 (whether `DESIGNING` should require `SCOPED`'s `REQUIREMENTS.md` even when `RESEARCHING` is reached directly from `DISCOVERY`) can't be resolved until Research (Phase 2) and Architecture (Phase 3) both exist to actually test the join against.
