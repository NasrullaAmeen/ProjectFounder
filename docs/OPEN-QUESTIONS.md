...
| Q9 | § 123's own worked classification (Step 3) uses labels — "Search," "Data Platform," bare "AI," "Semantic Search" — that aren't in § 28's taxonomy (confirmed against `config/project-types.yaml` while building `examples/bookmark-manager/`, see its `NOTES.md`). Should § 28 gain these values, should § 123 be treated as loosely illustrative rather than literal, or should each dimension get a free-text `other` escape hatch? | `open` | Any future classification of a project whose real shape doesn't fit the current 8/7/8/8/6 taxonomy values |
