...
This repository is currently a **v0.1 scaffold**, with Phase 0 (Foundation) complete for its declared scope and Phase 1 (Intelligence Core) underway: the Project Engine has a working, agent-executed slice — `commands/new-project.md` → `workflows/new-project.md` → `agents/project-architect.md` — that founds and classifies a new project up through `PROJECT.yaml` at lifecycle `CLASSIFIED`; the Discovery and Brainstorm Engines then take it further — `commands/brainstorm.md` → `workflows/brainstorm.md` → `agents/discovery-agent.md`/`agents/brainstorm-agent.md` — surfacing critical assumptions/constraints/open questions and a draft feature candidate list, up through lifecycle `DISCOVERY`. Both run as coding-agent-followed contracts, not as code (see `docs/DECISIONS.md` D005). Requirements, Feature Discovery, and Gap Analysis (the rest of Phase 1) and everything past that (Research onward) are still specification only — see `ProjectFounder-idea.md` § *v0.1 Minimum Viable Definition* and § *v0.1 Implementation Phases* for the build order, and `docs/LIMITATIONS.md` for what doesn't work yet.
