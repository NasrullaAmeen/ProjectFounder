...
This repository is currently a **v0.1 scaffold**, with Phase 0 (Foundation) complete for its declared scope and Phase 1 (Intelligence Core) fully wired end-to-end: the Project Engine founds and classifies a new project up through `PROJECT.yaml` at lifecycle `CLASSIFIED` (`commands/new-project.md` → `workflows/new-project.md` → `agents/project-architect.md`); the Discovery and Brainstorm Engines surface critical assumptions/constraints/open questions and a draft feature list, up through lifecycle `DISCOVERY` (`commands/brainstorm.md` → `workflows/brainstorm.md`); the Gap Analysis, Feature Discovery, and Requirements Engines then fill in missed features, tier everything into MVP/V1/V2/Future, and derive requirements, up through lifecycle `SCOPED` (`commands/requirements.md` → `workflows/requirements.md`). All three run as coding-agent-followed contracts, not as code (see `docs/DECISIONS.md` D005). Everything past that (Research onward, Phase 2+) is still specification only — see `ProjectFounder-idea.md` § *v0.1 Minimum Viable Definition* and § *v0.1 Implementation Phases* for the build order, and `docs/LIMITATIONS.md` for what doesn't work yet.
